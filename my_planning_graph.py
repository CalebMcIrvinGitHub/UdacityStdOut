...
class ActionLayer(BaseActionLayer):
    ...
    def _inconsistent_effects(self, actionA, actionB):
        ...
        for iterA in self.children[actionA]:
            for iterB in self.children[actionB]:
                print(type(iterA))
                print(type(iterB))
                if iterA == ~iterB:
                    return True
    ...
    def _interference(self, actionA, actionB):
        ...
        for parA in self.parents[actionA]:
            for chilB in self.children[actionB]:
                if parA == ~chilB:
                    return True
                        
        for parB in self.parents[actionB]:
            for chilA in self.children[actionA]:
                if parB == ~chilA:
                    return True
...
class PlanningGraph:
    ...
    def h_maxlevel(self):
        ...
        max_level = 0
        for l in self.goal:
           max_level = max(max_level, self.find_goal(g))
        return max_level
